...
@SearchTags({"potato"})
public final class PotatoauraHack extends Hack
	implements UpdateListener, RenderListener
{
	...
	private final HashMap<Block, Item> potato = new HashMap<>();
	{
		potato.put(Blocks.POTATOES, Items.POTATO);
	}
	...
	private void updatePlants(List<BlockPos> blocks)
	{
		...
		{
			Item seed = potato.get(BlockUtils.getBlock(pos));
			if(seed == null)
				continue;
	...
	private boolean shouldBeHarvested(BlockPos pos)
	{
		...
		if(block instanceof PotatoesBlock)
			return ((PotatoesBlock)block).isMature(state);
	...
	private boolean replant(List<BlockPos> blocksToReplant)
	{
		...
		// check if already holding one of the potato needed for blocksToReplant
		...
		// otherwise, find a block that we can reach and have potato for
