...
public final class ElytraLockFilterList extends EntityFilterList
{
	...
	public static ElytraLockFilterList create()
	{
		...
		builder.add(new FilterNeutralSetting(
			"Won't target neutral mobs like"
				+ " endermen and wolves when locking in.",
		...
		builder.add(new FilterPassiveSetting("Won't target animals like pigs"
			...
			+ " squid and dolphins when locking in.", true));
		...
		builder.add(new FilterSlimesSetting(
			"Won't target slimes when" + " locking in.", true));
		
		builder.add(new FilterVillagersSetting("Won't target villagers and"
			+ " wandering traders when locking in.", true));
		
		builder.add(new FilterZombieVillagersSetting(
			"Won't target zombified" + " villagers when locking in.", true));
		
		builder.add(new FilterGolemsSetting(
			"Won't target iron golems and snow" + " golems when locking in.",
			true));
		
		builder.add(new FilterPiglinsSetting(
			"Won't target piglins when" + " locking in.",
			AttackDetectingEntityFilter.Mode.ON));
		
		builder.add(new FilterZombiePiglinsSetting(
			"Won't target" + " zombified piglins when locking in.",
			AttackDetectingEntityFilter.Mode.ON));
		
		builder.add(new FilterShulkersSetting(
			"Won't target shulkers when" + " locking in.", true));
		
		builder.add(new FilterAllaysSetting(
			"Won't target allays when locking in.", true));
